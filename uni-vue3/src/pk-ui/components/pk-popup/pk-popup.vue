...
import { metaProvideSymbol, type IMetaProvide } from '../../libs/utils'
...
//处理返回键逻辑
const { registerPressback, unRegisterPressback } = inject<IMetaProvide>(metaProvideSymbol, {
    registerPressback: () => { },
    unRegisterPressback: () => { }
})
...
watchEffect(() => {
    if (show.value) {
        registerPressback(getHide)
    } else {
        unRegisterPressback(getHide)
    }
})
